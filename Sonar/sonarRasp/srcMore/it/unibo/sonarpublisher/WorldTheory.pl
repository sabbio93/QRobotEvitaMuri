%==============================================
% WorldTheory.pl for actor sonarpublisher
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatusonarpublisher).	%%old version (deprecated)
actorobj(qatusonarpublisher).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor sonarpublisher
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of sonarpublisher" ). 
...
%-------------------------------------------------
%  User static rules about sonarpublisher
%------------------------------------------------- 
mqttServer( "tcp://m2m.eclipse.org:1883").
topic( "unibo/mqtt/radar").
config( MQTTSERV,TOPIC):-mqttServer( MQTTSERV),topic( TOPIC).
valToPublish( TOPIC,p( Distance,ANGLE)):-topic( TOPIC),retract( msg( MSGID,_,SENDER,none,p( Distance,SONARID),SEQNUM)),mapToAngle( SONARID,ANGLE),actorPrintln( valToPublishp( Distance,ANGLE)).
mapToAngle( sonar1,30).
mapToAngle( sonar2,60).
mapToAngle( ANY,90).
/*
------------------------------------------------------------------------
